...
class INV_block(nn.Module):
    ...
    def forward(self, x, rev=False):
        ...
        # GLOBAL_ITEM.outs.append((y1, y2))
        return torch.cat((y1, y2),    1)


class INV_block_v2(nn.Module):
    def __init__(self, subnet_constructor=ResidualDenseBlock_out, clamp=2, in_1=3, in_2=None):
        super().__init__()
        if in_2 is None:
            in_2 = in_1
    ...
    def forward(self, x: torch.Tensor, rev=False):
        x1, x2 = (x.narrow(1, 0, self.split_len1),
                  x.narrow(1, self.split_len1, self.split_len2))
